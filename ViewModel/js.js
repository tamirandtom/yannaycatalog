...
function faceTrackRectangle(ev) {
	...
	if (ev.detection == "CS") {
		...
		overlayContext.translate(-ev.x, -ev.y);
		// $("#main-content").css({"top":(ev.y*2)+"px","left":(ev.x*2)+"px"});
		$("#main-content").css("top",((screen.width/3)-(ev.y*3))+"px");
		document.getElementById('divisionatio2').innerHTML = "div.y:"+((screen.width/3)-(ev.y*3));
	}
...
function breakPointClass(ev) {
    ...
    var faceWidth   = ev.width,
        videoWidth  = videoInput.width,
        face2canvasRatio = (videoWidth/faceWidth)*10; 
    	$("p").css({"font-size":face2canvasRatio+"px","line-height":180-(face2canvasRatio)+"%"});
    	document.getElementById('divisionatio').innerHTML = "div.font-size:"+Math.round(face2canvasRatio);
